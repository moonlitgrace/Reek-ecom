...
const SavedItem = ({ item, getSavedItems, getBasketItems, getBasket }) => {
    ...
    const handleRemoveSaved = async () => {
        let url = `${BackendUrl}/api/saved/remove/`
    ...
    const handleAddtoCart = async () => {
        let url = `${BackendUrl}/api/addtocart/`
    ...
    return (
        <div className='items grid grid-cols-7 gap-5 py-3'>
            <div className="item-img-title flex flex-col md:flex-row md:col-span-4 col-span-7">
                <div className='grid grid-cols-7 gap-3 md:gap-0'>
                    ...
                    <Link to={`/item/${item.item.slug}`} className='md:col-span-4 col-span-5'>
                        <h3 className='font-medium hover:underline cursor-pointer'>{truncate(item.item.title)}</h3>
                ...
                <div className="mobile_view md:hidden grid grid-cols-7 w-full mt-2 text-sm gap-3 flex items-center">
                    ...
                    <button onClick={handleRemoveSaved} className='hover:underline text-red-800 hover:text-red-900'>Remove</button>
                    </div>
                </div>
            </div>
            ...
            <div className="item-price hidden md:block">
                <h4>₹ {item.total}</h4>
                <h4 className='text-sm text-stone-500'><strike>₹ {item.real_total}</strike></h4>
            </div>
            <div className="remove hidden md:block">
                <button onClick={handleAddtoCart} className='hover:underline'>Add to cart</button>
                <button onClick={handleRemoveSaved} className='hover:underline text-red-800 hover:text-red-900'>Remove</button>
            </div>
        </div>
    )
